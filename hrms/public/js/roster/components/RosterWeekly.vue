...
const onPaste = (evt) => {
  ...
  for (const element of rows) {
    ...
    const f = cols.filter(
      (c) => ["r", "d", "n"].indexOf(c.toLowerCase()) !== -1
    );
    if (f.length !== 7) {
      return;
    }
...
function generateRoster(numEmployees) {
  ...
  const roster = Array.from({ length: numEmployees }, () =>
    Array(numDays).fill("D")
  );
          ...
          <div class="frappe-control input-max-width">
            <div class="form-group">
              ...
              <input
                ...
                @input="
                  (event) => onNumberOfEmployeesChange(event.target.value)
                "
              />
            </div>
          </div>
        ...
        <div class="frappe-control input-max-width">
          <div class="form-group">
            ...
            <button
              ...
              @click="generateRoster(numberOfEmployees)"
            >
              Random
            </button>
          </div>
