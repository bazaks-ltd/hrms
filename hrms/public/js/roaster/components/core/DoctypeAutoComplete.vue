...
const props = defineProps(['modelValue', 'doctype', 'title'])
...
onMounted(() => {
	frappe.db.get_list(props.doctype, {
		limit: 0
	}).then(res => {
		lists.value = res.map(r => r.name)
...
watch(inputText, () => {
	...
	filteredList.value = o.filter(f => {
		return f.toLowerCase().startsWith(inputText.value.toLowerCase())
	})
})
...
<template>
	<div class="form-column col-sm-4">
		<form>
			<div class="frappe-control " data-fieldtype="Link" data-fieldname="department">
				<div class="form-group">
					<div class="clearfix">
						<label class="control-label">{{ props.title }}</label>

					</div>
					<button v-if="model" class="btn btn-primary" type="button"
						@click="model = null; $emit('update:modelValue', undefined)">{{ model }} <i
