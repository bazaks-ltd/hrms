...
frappe.ui.form.on("New Joiner Salary Structure Assignment", {
	setup(frm) {
	...
	async refresh(frm) {
		...
		frm.trigger("set_refresh_button");
	...
	render_employees_datatable(frm, employees) {
		...
		const no_data_message = __(
			"There are no employees without a Salary Structure Assignment based on the given filters."
		);
	...
	show_confirm_dialog(frm, checked_rows_content) {
		frappe.confirm(
			__("Assign Salary Structure to {0} employee(s)? From Date will be each employee's Date of Joining.", [
